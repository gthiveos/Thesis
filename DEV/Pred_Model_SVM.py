...
sent_detector = LinearSVC().fit(messages_tfidf, messages['sentiment'])
...
msg_train, msg_test, label_train, label_test = \
    train_test_split(messages['message'], messages['label'], test_size=0.1)
	
msg_train_1, msg_test_1, label_train_1, label_test_1 = \
    train_test_split(messages['message'], messages['sentiment'], test_size=0.1)
...
grid_svm_1 = GridSearchCV(
    pipeline_svm,  # pipeline from above
    param_grid=param_svm,  # parameters to tune via cross validation
    refit=True,  # fit using all data, on the best detected classifier
    n_jobs=1,  # number of cores to use for parallelization; -1 for "all cores"
    scoring='accuracy',  # what score are we optimizing?
    cv=StratifiedKFold(label_train_1, n_folds=10),  # what type of cross validation to use
)
...
svm_detector_1 = grid_svm_1.fit(msg_train_1, label_train_1) # find the best combination from param_svm
print (svm_detector_1.grid_scores_)
...
predictions_1 = svm_detector_1.predict(msg_test_1)
print (confusion_matrix(label_test_1, svm_detector_1.predict(msg_test_1)))
print (classification_report(label_test_1, svm_detector_1.predict(msg_test_1)))

print("Accuracy on test set:  %.2f%%" % (100 * (svm_detector_1.grid_scores_[0][1])))
...
print("Tweet #1:", "'",my_1st_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_1st_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_1st_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_1st_tweet])[0])), svm_detector_1.predict([my_1st_tweet])[0])
print("Tweet #2:", "'",my_2nd_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_2nd_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_2nd_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_2nd_tweet])[0])), svm_detector_1.predict([my_2nd_tweet])[0])
print("Tweet #3:", "'",my_3rd_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_3rd_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_3rd_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_3rd_tweet])[0])), svm_detector_1.predict([my_3rd_tweet])[0])
print("Tweet #4:", "'",my_4th_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_4th_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_4th_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_4th_tweet])[0])), svm_detector_1.predict([my_4th_tweet])[0])
print("Tweet #5:", "'",my_5th_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_5th_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_5th_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_5th_tweet])[0])), svm_detector_1.predict([my_5th_tweet])[0])
print("Tweet #6:", "'",my_6th_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_6th_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_6th_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_6th_tweet])[0])), svm_detector_1.predict([my_6th_tweet])[0])
print("Tweet #7:", "'",my_7th_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_7th_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_7th_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_7th_tweet])[0])), svm_detector_1.predict([my_7th_tweet])[0])
print("Tweet #8:", "'",my_4th_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_8th_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_8th_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_8th_tweet])[0])), svm_detector_1.predict([my_8th_tweet])[0])
print("Tweet #9:", "'",my_9th_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_9th_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_9th_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_9th_tweet])[0])), svm_detector_1.predict([my_9th_tweet])[0])
print("Tweet #10:", "'",my_10th_tweet, "'", ' \n \n', "I'm about %.0f%%" % (100 * max(svm_detector.predict_proba([my_10th_tweet])[0])), "sure this was tweeted by", svm_detector.predict([my_10th_tweet])[0], "and the polarity is about %.0f%%" % (100 * max(svm_detector_1.predict_proba([my_10th_tweet])[0])), svm_detector_1.predict([my_10th_tweet])[0])

#Popular tweets for Trump probability
